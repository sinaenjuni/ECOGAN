...
g_bn = ConditionalBatchNorm2d
g_embedding = sn_embedding


d_conv2d = snconv2d
d_linear = snlinear
d_act_fn = nn.ReLU(True)
d_embedding = sn_embedding
...
class GenBlock(nn.Module):
    def __init__(self, in_channels, out_channels, affine_input_dim):
        ...
        self.bn1 = g_bn(affine_input_dim, in_channels)
        self.bn2 = g_bn(affine_input_dim, out_channels)

        self.activation = g_act_fn
        self.conv2d0 = g_conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0)
        self.conv2d1 = g_conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)
        self.conv2d2 = g_conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)
...
class Generator(nn.Module):
    def __init__(self, z_dim, g_shared_dim, img_size, img_dim, g_conv_dim, num_classes, g_init, **kwargs):
        ...
        self.linear0 = g_linear(in_features=self.chunk_size, out_features=self.in_dims[0]*self.bottom*self.bottom, bias=True)

        # if self.g_cond_mtd != "W/O":
        self.affine_input_dim += self.g_shared_dim
        self.shared = g_embedding(num_embeddings=self.num_classes, embedding_dim=self.g_shared_dim)
        ...
        self.activation = g_act_fn
        self.conv2d5 = g_conv2d(in_channels=self.out_dims[-1], out_channels=self.img_dim, kernel_size=3, stride=1, padding=1)
        self.tanh = nn.Tanh()
...
# g_init = 'ortho'
#
# G = Generator(z_dim, g_shared_dim, img_size, img_dim, g_conv_dim, num_classes, g_init)
#
# z = torch.randn(128, 80)
...
class DiscOptBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv2d0 = d_conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0)
        self.conv2d1 = d_conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)
        self.conv2d2 = d_conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)


        self.activation = d_act_fn
...
class Discriminator(nn.Module):
    def __init__(self, img_size, img_dim, d_conv_dim, d_embed_dim, normalize_d_embed, num_classes, d_init):
        super(Discriminator, self).__init__()
        d_in_dims_collection = {
            "32": [img_dim] + [d_conv_dim * 2, d_conv_dim * 2, d_conv_dim * 2],
            "64": [img_dim] + [d_conv_dim, d_conv_dim * 2, d_conv_dim * 4, d_conv_dim * 8],
            "128": [img_dim] + [d_conv_dim, d_conv_dim * 2, d_conv_dim * 4, d_conv_dim * 8, d_conv_dim * 16],
            "256": [img_dim] + [d_conv_dim, d_conv_dim * 2, d_conv_dim * 4, d_conv_dim * 8, d_conv_dim * 8, d_conv_dim * 16],
            "512": [img_dim] + [d_conv_dim, d_conv_dim, d_conv_dim * 2, d_conv_dim * 4, d_conv_dim * 8, d_conv_dim * 8, d_conv_dim * 16]
        }
        ...
        self.linear1 = d_linear(in_features=self.out_dims[-1], out_features=1, bias=True)
        ...
        self.linear2 = d_linear(in_features=self.out_dims[-1], out_features=d_embed_dim, bias=True)
        self.embedding = d_embedding(num_classes, d_embed_dim)
...
# d_init = 'ortho'
#
# D = Discriminator(img_size, img_dim, d_conv_dim, d_embed_dim, normalize_d_embed, num_classes, d_init)
#
# z = torch.randn(128, 80)
# label = torch.randint(0, 10, (128,))
# y = G(z, label)
