...
from gen_data.utils.style_ops import conv2d_gradfix
...
#
#         f2f_sim = self.calculate_similarity_matrix(embed, embed)
...
#
#         pos_mask_redia = self._remove_diag(self._make_neg_removal_mask(label)[label])
#         f2f_logits_pos_only = pos_mask_redia * f2f_logits
#
#         # emb2proxy = torch.exp(self.cosine_similarity(embed, proxy) / self.temperature)
#
#         e2p_sim = self.calculate_similarity_matrix(embed, proxy)
...
#
#         pos_mask = self._make_neg_removal_mask(label)[label]
